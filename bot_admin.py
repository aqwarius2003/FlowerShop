...
from telegram.ext import (
    CallbackContext, CallbackQueryHandler, MessageHandler, Filters, CommandHandler,
)
...
def delivery_orders(update: Update, context: CallbackContext):
    if update.message:
        ...
        return  # Добавить проверка если id доставщика не найдено
...
def handle_order_selection(update: Update, context: CallbackContext):
    logger.info("Вошли в handle_order_selection")
    query = update.callback_query
...
def get_handlers():
    # Command handlers
    # start_handler = CommandHandler('start', start)  # Если у вас есть функция start
    delivery_orders_handler = CommandHandler('delivery_orders', delivery_orders)

    # Callback query handlers
    handle_delivery_order_handler = CallbackQueryHandler(handle_delivery_order, pattern=r'^order_delivery_\d+$')
    go_back_to_delivery_orders_handler = CallbackQueryHandler(go_back_to_delivery_orders, pattern=r'^back_to_delivery_orders$')
    set_delivery_status_handler = CallbackQueryHandler(set_delivery_status, pattern=r'^setDeliveryStatus_\d+_\w+$')
    handle_order_selection_handler = CallbackQueryHandler(handle_order_selection, pattern=r'^order_admin_\d+$')
 ...
 #   add_comment_handler = CallbackQueryHandler(add_comment, pattern=r'^comment_delivery_\d+$')
    ...
    # handle_comment_input_handler = MessageHandler(Filters.text & ~Filters.command, handle_comment_input)

    return [
        # start_handler,
        delivery_orders_handler,
        handle_delivery_order_handler,
        go_back_to_delivery_orders_handler,
        set_delivery_status_handler,
        handle_order_selection_handler,
