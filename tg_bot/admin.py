...
from .models import UserBot, Category, Product, Order, PriceRange


@admin.register(UserBot)
class UserBotAdmin(admin.ModelAdmin):
    list_display = ('user_id', 'full_name', 'phone', 'address', 'status')  # Параметры для отображения в списке
    search_fields = ('user_id', 'full_name', 'phone')  # Поля, по которым будет происходить поиск
...
class ProductAdmin(admin.ModelAdmin):
    ...
    filter_horizontal = ('categories',)  # Для удобного выбора категорий в ManyToMany поле


@admin.register(Order)
class OrderAdmin(admin.ModelAdmin):
    list_display = ('product', 'user', 'status', 'creation_date', 'desired_delivery_date', 'manager', 'delivery_person')
